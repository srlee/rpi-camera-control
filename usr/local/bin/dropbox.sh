...
function db_stat
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" "$API_METADATA_URL/$ACCESS_LEVEL/$(urlencode "$FILE")?oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" 2> /dev/null
    check_http_response
...
function db_chunked_upload_file
{
    ...
    while ([[ $OFFSET != $FILE_SIZE ]]); do
        ...
        #Uploading the chunk...
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --upload-file "$CHUNK_FILE" "$API_CHUNKED_UPLOAD_URL?$CHUNK_PARAMS&oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" 2> /dev/null
        check_http_response
    ...
    while (true); do
        ...
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "upload_id=$UPLOAD_ID&oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_CHUNKED_UPLOAD_COMMIT_URL/$ACCESS_LEVEL/$(urlencode "$FILE_DST")" 2> /dev/null
        check_http_response
...
{
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_INFO_URL" 2> /dev/null
    check_http_response
...
function db_download_file
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES $CURL_PARAMETERS --globoff -D "$RESPONSE_FILE" -o "$FILE_DST" "$API_DOWNLOAD_URL/$ACCESS_LEVEL/$(urlencode "$FILE_SRC")?oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM"
    check_http_response
...
function db_delete
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM&root=$ACCESS_LEVEL&path=$(urlencode "$FILE_DST")" "$API_DELETE_URL" 2> /dev/null
    check_http_response
...
function db_move
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM&root=$ACCESS_LEVEL&from_path=$(urlencode "$FILE_SRC")&to_path=$(urlencode "$FILE_DST")" "$API_MOVE_URL" 2> /dev/null
    check_http_response
...
function db_copy
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM&root=$ACCESS_LEVEL&from_path=$(urlencode "$FILE_SRC")&to_path=$(urlencode "$FILE_DST")" "$API_COPY_URL" 2> /dev/null
    check_http_response
...
function db_mkdir
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM&root=$ACCESS_LEVEL&path=$(urlencode "$DIR_DST")" "$API_MKDIR_URL" 2> /dev/null
    check_http_response
...
function db_list
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" "$API_METADATA_URL/$ACCESS_LEVEL/$(urlencode "$DIR_DST")?oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" 2> /dev/null
    check_http_response
...
function db_share
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" "$API_SHARES_URL/$ACCESS_LEVEL/$(urlencode "$FILE_DST")?oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$DROPBOX_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$DROPBOX_ACCESS_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM&short_url=false" 2> /dev/null
    check_http_response
...
if [[ -e $CONFIG_FILE ]]; then
    ...
    if [[ $DROPBOX_API_KEY == "" || $DROPBOX_API_SECRET == "" || $DROPBOX_ACCESS_TOKEN_SECRET == "" || $DROPBOX_ACCESS_TOKEN == "" ]]; then
...
#NEW SETUP...
    ...
    while (true); do
        ...
        echo -ne "\n > App key is $DROPBOX_API_KEY, App secret is $DROPBOX_API_SECRET and Access level is $ACCESS_MSG. Looks ok? [y/n]: "
        read answer
        if [[ $answer == "y" ]]; then
            break;
        fi
    ...
    #TOKEN REQUESTS
    echo -ne "\n > Token request... "
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$DROPBOX_API_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_REQUEST_TOKEN_URL" 2> /dev/null
    check_http_response
    ...
    while (true); do
        ...
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$DROPBOX_API_KEY&oauth_token=$OAUTH_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$DROPBOX_API_SECRET%26$OAUTH_TOKEN_SECRET&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_ACCESS_TOKEN_URL" 2> /dev/null
        check_http_response
        ...
        if [[ $DROPBOX_ACCESS_TOKEN != "" && $DROPBOX_ACCESS_TOKEN_SECRET != "" && $OAUTH_ACCESS_UID != "" ]]; then
            echo -ne "OK\n"

            #Saving data in new format, compatible with source command.
            echo "APPKEY=$DROPBOX_API_KEY" > "$CONFIG_FILE"
            echo "APPSECRET=$DROPBOX_API_SECRET" >> "$CONFIG_FILE"
            echo "ACCESS_LEVEL=$ACCESS_LEVEL" >> "$CONFIG_FILE"
            echo "OAUTH_ACCESS_TOKEN=$DROPBOX_ACCESS_TOKEN" >> "$CONFIG_FILE"
            echo "OAUTH_ACCESS_TOKEN_SECRET=$DROPBOX_ACCESS_TOKEN_SECRET" >> "$CONFIG_FILE"
